...
var push_col_left, push_col_right;
push_col_left  = player_collision_check(COL_LEFT,  MASK_BIG, x, y, angle);
push_col_right = player_collision_check(COL_RIGHT, MASK_BIG, x, y, angle);
...
if (ground && y_speed == 0)
{
    if (state != STATE_SPINDASH && state != STATE_LOOKUP && state != STATE_CROUCH)
    {
        ...
        {
            if ((push_col_right && input_right) || (push_col_left && input_left))
            {
                push_timer += 0.5;
            }
        }
