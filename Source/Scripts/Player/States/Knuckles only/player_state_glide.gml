/// player_state_glide()
// Script to handle Knuckles' gliding.
...
if (state == STATE_GLIDE_DROP && ground)
{
    ...
    if ((angle >= 0 && angle < 25) || (angle <= 360 && angle > 345))
    {
        ...
        if (animation != "CROUCH")
        {
            audio_play_sound(SFX._player_land, global.sfx_volume, 1, 0, 1);
        }
        animation = "CROUCH";
    ...
    else
    {
        if (abs(x_speed) > 1)
        {
            state              = STATE_ROLL;
            glide_crouch_timer = 15;
            audio_play_sound(SFX._player_roll, global.sfx_volume, 1, 0, 1);
        }
        else
        {
            state              = STATE_DEFAULT;
...
if (state == STATE_JUMP && input_action_pressed && shield_usable)
{
    ...
    // Reset vertical speed if it's below 0.
...
if (state == STATE_GLIDE)
{
    if (((player_calculate_angle(x, y, angle) == 0) && x_speed >= 0 && player_collision_check(COL_RIGHT, MASK_BIG, x, y, 0) && player_collision_check(COL_RIGHT, spr_player_mask_ledge_climb, x, y, 0)) && !player_collision_check(COL_RIGHT_OBJECT, MASK_BIG, x, y, 0, obj_monitor) && !player_collision_check(COL_RIGHT_OBJECT, MASK_BIG, x, y, 0, par_spike) && !player_collision_check(COL_TOP, MASK_MAIN, x, y, 0))
    || (((player_calculate_angle(x, y, angle) == 0) && x_speed <= 0 && player_collision_check(COL_LEFT,  MASK_BIG, x, y, 0) && player_collision_check(COL_LEFT,  spr_player_mask_ledge_climb, x, y, 0)) && !player_collision_check(COL_LEFT_OBJECT,  MASK_BIG, x, y, 0, obj_monitor) && !player_collision_check(COL_LEFT_OBJECT,  MASK_BIG, x, y, 0, par_spike) && !player_collision_check(COL_TOP, MASK_MAIN, x, y, 0))
    {
        ...
        // Play sound.
        audio_play_sound(SFX._player_hang, global.sfx_volume, 1, 0, 1);
    }
    else if (((player_calculate_angle(x, y, angle) != 0) && x_speed >= 0 && player_collision_check(COL_RIGHT, MASK_BIG, x, y, 0) && player_collision_check(COL_RIGHT, spr_player_mask_ledge_climb, x, y, 0)) && !player_collision_check(COL_RIGHT_OBJECT, MASK_BIG, x, y, 0, obj_monitor) && !player_collision_check(COL_RIGHT_OBJECT, MASK_BIG, x, y, 0, par_spike) && !player_collision_check(COL_TOP, MASK_MAIN, x, y, 0))
    ||      (((player_calculate_angle(x, y, angle) != 0) && x_speed <= 0 && player_collision_check(COL_LEFT,  MASK_BIG, x, y, 0) && player_collision_check(COL_LEFT,  spr_player_mask_ledge_climb, x, y, 0)) && !player_collision_check(COL_LEFT_OBJECT,  MASK_BIG, x, y, 0, obj_monitor) && !player_collision_check(COL_LEFT_OBJECT,  MASK_BIG, x, y, 0, par_spike) && !player_collision_check(COL_TOP, MASK_MAIN, x, y, 0))
    {
        state        = STATE_ROLL;
        glide_turn_s = 0;
