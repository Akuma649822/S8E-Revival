...
if (state == STATE_SPINDASH)
{
    ...
    {
        if ((animation_direction == 1 && (!player_collision_check(COL_RIGHT, MASK_BIG, x, y, angle) || player_collision_check(COL_RIGHT_OBJECT, MASK_BIG, x, y, angle, obj_monitor))) || (animation_direction == -1 && (!player_collision_check(COL_LEFT, MASK_BIG, x, y, angle)) || player_collision_check(COL_LEFT_OBJECT, MASK_BIG, x, y, angle, obj_monitor)))
        {
            state              = STATE_ROLL;
    ...
    // Make sure there's no horizontal speed:
    ...
    if (input_action_pressed)
    {
        ...
        // Set pitch and play charging sound:
        ...
        audio_play_sound(SFX._player_spindash_charge, global.sfx_volume, 1 + (spindash_pitch * 0.075), 0, 0);
...
if (state == STATE_CROUCH && input_action_pressed)
{
    ...
    spindash_strength = 0;
    audio_play_sound(SFX._player_spindash_charge, global.sfx_volume, 1, 0, 0);
}
